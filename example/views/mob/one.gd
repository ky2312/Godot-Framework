...
func _ready() -> void:
	...
	var mob_model := GameManager.app.get_container(MobModelNS.IMobModel) as MobModelNS.IMobModel
	# 两种使用方式
	# 便捷使用多个值
	FrameworkBindableProperty.register_propertys_with_init_value(
		[player_model.get_kill_count(), player_model.get_achievement_kill_count(), mob_model.get_count()],
	...
	# 通常使用单个值
	#player_model.get_kill_count().register_with_init_value(
		#func(kill_count):
			#kill_label.text = "已击杀 {0} 次，".format([kill_count])
	#).unregister_when_node_exit_tree(self)
	#player_model.get_achievement_kill_count().register_with_init_value(
		#func(achievement_kill_count):
			#mob_label.text = "上次存档点已击杀 {0} 次。".format([achievement_kill_count])
	#).unregister_when_node_exit_tree(self)
	#mob_model.get_count().register_with_init_value(
		#func(count):
			#kill_button.text = "点击击杀1只小怪，剩余 {0} 只".format([count])
	#).unregister_when_node_exit_tree(self)
